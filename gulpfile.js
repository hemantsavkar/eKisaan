...
gulp.task('clean', function (cb) {
  return gulp.src([
        ...
        path.join(config.dest, 'loadTemplate'),
        path.join(config.dest, 'localdata'),
        path.join(config.dest, 'libs')
      ], { read: false })
     .pipe(rimraf());
});
...
gulp.task('loadTemplateCopy', function() {
  return gulp.src(config.vendor.loadTemplate)
  .pipe(gulp.dest(path.join(config.dest, 'loadTemplate')));
});
/*==================================
=            Copy localdata            =
==================================*/

gulp.task('localdataCopy', function() {
  return gulp.src(config.vendor.localData)
  .pipe(gulp.dest(path.join(config.dest, 'localdata')));
});
...
gulp.task('libsCopy', function() {
  return gulp.src(config.vendor.libs)
...
gulp.task('js', function() {
    streamqueue({ objectMode: true },
      ...
      gulp.src('./src/js/**/*.js').pipe(ngFilesort()),
      gulp.src(['src/templates/**/*.html']).pipe(templateCache({ module: 'kissanApp' }))
    )
...
gulp.task('watch', function () {
  ...
  gulp.watch(['./src/libs/**/*'], ['libsCopy']);
    gulp.watch(['./src/localData/**/*'], ['localDataCopy']);
    gulp.watch(['./src/loadTemplate/**/*'], ['loadTemplateCopy']);
});
...
gulp.task('build', function(done) {
  var tasks = ['html', 'fonts', 'less', 'js','loadTemplateCopy','localdataCopy','libsCopy'];
